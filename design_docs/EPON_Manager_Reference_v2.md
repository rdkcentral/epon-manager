# EPON Manager — TR-181, Telemetry & Harvester Reference
...
3. [Harvester Periodic Report (Avro)](#3-harvester-periodic-report-avro)
   1. [Report Fields](#31-report-fields)
   2. [Avro Schema](#32-avro-schema-eponreportavsc)
...
> **Note:** Periodic statistics telemetry (formerly Section 6 in the original proposal) has been replaced by the Harvester Avro report described in [Section 3](#3-harvester-periodic-report-avro).
...
> **Note:** Each alarm event carries a raised/cleared state in its value. The priority listed is for the raised condition; cleared events are informational. Periodic statistics (28 events) have been migrated to the Harvester Avro report ([Section 3](#3-harvester-periodic-report-avro)).

---

## 3. Harvester Periodic Report (Avro)

All periodic statistics formerly reported as individual T2 telemetry markers have been consolidated into a single **Harvester Avro report** named `EPONTelemetryDiagnostics`. The report is generated by the stats poller at a configurable interval (default: **900 seconds / 15 minutes**) and published as a binary Avro record to the Harvester pipeline.

> **Schema namespace:** `com.comcast.kestrel.odp.event`  
> **Record name:** `EPONTelemetryDiagnostics`

---

### 3.1 Report Fields

The report contains a standard Kestrel header, CPE identification, and the EPON telemetry data payload. All data fields are nullable to handle cases where a value is unavailable.

#### 3.1.1 Header & CPE Identification

| Field | Description | Avro Type |
|-------|-------------|-----------|
| `header.timestamp` | UTC timestamp in milliseconds since Unix epoch when the report is generated. | union[null, long (timestamp-millis)] |
| `header.uuid` | Unique identifier (16-byte fixed) for de-duplication and tracing. | union[null, fixed(16)] |
| `header.source` | Source application generating the report (e.g., _rdk-eponmanager_). | union[null, string] |
| `cpe_id.mac_address` | Canonical (identifying) MAC address of the gateway (6-byte fixed). | union[null, fixed(6)] |
| `cpe_id.cpe_type` | CPE type, e.g., _Gateway_ or _Extender_. | union[null, string] |

#### 3.1.2 Link Statistics

Source: `epon_hal_get_link_stats()`

| Field | Description | Avro Type | Unit |
|-------|-------------|-----------|------|
| `data.BytesSent` | Total bytes transmitted on the EPON optical interface (TR-181: `Stats.BytesSent`). | union[null, long] | bytes |
| `data.BytesReceived` | Total bytes received on the EPON optical interface (TR-181: `Stats.BytesReceived`). | union[null, long] | bytes |
| `data.PacketsSent` | Total packets transmitted (TR-181: `Stats.PacketsSent`). | union[null, long] | packets |
| `data.PacketsReceived` | Total packets received (TR-181: `Stats.PacketsReceived`). | union[null, long] | packets |
| `data.ErrorsSent` | Total transmission errors (TR-181: `Stats.ErrorsSent`). | union[null, long] | count |
| `data.ErrorsReceived` | Total reception errors (TR-181: `Stats.ErrorsReceived`). | union[null, long] | count |
| `data.DiscardPacketsSent` | Packets discarded prior to transmission (TR-181: `Stats.DiscardPacketsSent`). | union[null, long] | packets |
| `data.DiscardPacketsReceived` | Packets discarded on reception (TR-181: `Stats.DiscardPacketsReceived`). | union[null, long] | packets |
| `data.BroadcastPacketsSent` | Broadcast packets transmitted (EPON extension). | union[null, long] | packets |
| `data.BroadcastPacketsReceived` | Broadcast packets received (EPON extension). | union[null, long] | packets |
| `data.MulticastPacketsSent` | Multicast packets transmitted (EPON extension). | union[null, long] | packets |
| `data.MulticastPacketsReceived` | Multicast packets received (EPON extension). | union[null, long] | packets |
| `data.MaxBitRate` | Maximum negotiated bit rate. Typically 1000 (1G-EPON) or 10000 (10G-EPON). | union[null, int] | Mbps |
| `data.FECCorrected` | FEC corrected bit error count (EPON extension). | union[null, long] | count |
...
### 3.2 Avro Schema (`EponReport.avsc`)

```json
{
    "namespace": "com.comcast.kestrel.odp.event",
    "name": "EPONTelemetryDiagnostics",
    "type": "record",
    "doc": "Real time diagnostics and telemetry periodic report to assess EPON ONU link health, optical transceiver performance, and interface status",
    "fields": [
        {
            "name": "header",
            "type": {
                ...
                "name": "CoreHeader",
                "type": "record",
                "doc": "Common information related to the event which MUST be included in any kestrel event.",
                "fields": [
                    {
                        "name": "timestamp",
                    ...
                    {
                        "name": "uuid",
                        "type": ["null", {"name": "UUID", "namespace": "com.comcast.kestrel.datatype", "size": 16, "type": "fixed"}],
        ...
        },
        {
            ...
            "type": {
                ...
                "name": "CPEIdentifier",
                "type": "record",
                "doc": "Unique identifying fields for a CPE device.",
                "fields": [
                    {
                        "name": "mac_address",
                        "type": ["null", {"name": "MacAddress", "namespace": "com.comcast.kestrel.datatype", "size": 6, "type": "fixed"}],
                        "doc": "Canonical (Identifying) MAC address for the gateway.",
                        "default": null
                    },
                    {
                        "name": "cpe_type",
        ...
        },
        {
            ...
            "type": {
                "namespace": "com.comcast.kestrel.odp",
                "name": "EPONTelemetryData",
                "type": "record",
                "doc": "Contains EPON ONU diagnostic and telemetry data collected from the HAL layer at a point in time",
                "fields": [
                    {"name": "BytesSent",               "type": ["null", "long"],   "doc": "Total bytes transmitted on the EPON optical interface (TR-181: Device.Optical.Interface.{i}.Stats.BytesSent)",                "default": null},
                    {"name": "BytesReceived",            "type": ["null", "long"],   "doc": "Total bytes received on the EPON optical interface (TR-181: Device.Optical.Interface.{i}.Stats.BytesReceived)",             "default": null},
                    {"name": "PacketsSent",              "type": ["null", "long"],   "doc": "Total packets transmitted on the EPON optical interface (TR-181: Device.Optical.Interface.{i}.Stats.PacketsSent)",           "default": null},
                    {"name": "PacketsReceived",          "type": ["null", "long"],   "doc": "Total packets received on the EPON optical interface (TR-181: Device.Optical.Interface.{i}.Stats.PacketsReceived)",          "default": null},
                    {"name": "ErrorsSent",               "type": ["null", "long"],   "doc": "Total transmission errors on the EPON optical interface (TR-181: Device.Optical.Interface.{i}.Stats.ErrorsSent)",            "default": null},
                    {"name": "ErrorsReceived",           "type": ["null", "long"],   "doc": "Total reception errors on the EPON optical interface (TR-181: Device.Optical.Interface.{i}.Stats.ErrorsReceived)",           "default": null},
                    {"name": "DiscardPacketsSent",       "type": ["null", "long"],   "doc": "Packets discarded prior to transmission (TR-181: Device.Optical.Interface.{i}.Stats.DiscardPacketsSent)",                   "default": null},
                    {"name": "DiscardPacketsReceived",   "type": ["null", "long"],   "doc": "Packets discarded on reception (TR-181: Device.Optical.Interface.{i}.Stats.DiscardPacketsReceived)",                        "default": null},
                    {"name": "BroadcastPacketsSent",     "type": ["null", "long"],   "doc": "Broadcast packets transmitted on the EPON optical interface (EPON extension)",                                               "default": null},
                    {"name": "BroadcastPacketsReceived", "type": ["null", "long"],   "doc": "Broadcast packets received on the EPON optical interface (EPON extension)",                                                  "default": null},
                    {"name": "MulticastPacketsSent",     "type": ["null", "long"],   "doc": "Multicast packets transmitted on the EPON optical interface (EPON extension)",                                               "default": null},
                    {"name": "MulticastPacketsReceived", "type": ["null", "long"],   "doc": "Multicast packets received on the EPON optical interface (EPON extension)",                                                  "default": null},
                    {"name": "MaxBitRate",               "type": ["null", "int"],    "doc": "Maximum negotiated bit rate in Mbps (TR-181: Device.Optical.Interface.{i}.MaxBitRate). Typically 1000 for 1G-EPON or 10000 for 10G-EPON", "default": null},
                    {"name": "FECCorrected",             "type": ["null", "long"],   "doc": "Number of FEC (Forward Error Correction) corrected bit errors on the EPON link (EPON extension)",                            "default": null},
